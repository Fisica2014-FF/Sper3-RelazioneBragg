...
my $operaz = qr/^|[\+\*\/\-\^\(\),\=]|$/; # Variabile regex che contiene i vari "delimitatori di variabile" (operazioni, parentesi, inizio e fine riga...)
...
# (?: ) = non capturing group
my $espressione = qr/(?:$operaz)?($numero|$variabile|(?R1))/;
